...
void OffscreenQmlSurface::create(QOpenGLContext* shareContext) {
    OffscreenGlSurface::create(shareContext);
...
QPointF OffscreenQmlSurface::mapWindowToUi(const QPointF& sourcePosition, QObject* sourceObject) {
    vec2 sizeXfm = vec2(qt::toGlm(_quickWindow->size()));
    if (dynamic_cast<QWidget*>(sourceObject)) {
        sizeXfm /= qt::toGlm(((QWidget*)sourceObject)->size());
    } else if (dynamic_cast<QWindow*>(sourceObject)) {
        sizeXfm /= qt::toGlm(((QWindow*)sourceObject)->size());
    }
    vec2 offscreenPosition = qt::toGlm(sourcePosition) * sizeXfm;
...
void OffscreenQmlSurface::releaseTexture(int texture, GLsync readSync) {
    _fboCache.releaseTexture(texture, readSync);
}
