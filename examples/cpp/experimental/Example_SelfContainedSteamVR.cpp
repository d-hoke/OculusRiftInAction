...
inline GLFWwindow * createVrRenderingWindow(IHmd * hmd, glm::uvec2 & outSize, glm::ivec2 & outPosition) {
    GLFWwindow * window = nullptr;
    bool directHmdMode = true;
    ivec2 pos; uvec2 size;
    hmd->GetWindowBounds(&outPosition.x, &outPosition.y, &outSize.x, &outSize.y);
    ...
    if (directHmdMode) {
        ...
        if (nullptr != nativeWindowHandle) {
            hmd->AttachToWindow(nativeWindowHandle);
        }
    }
...
public:
    SteamVrApp() {
        SetEnvironmentVariableA("VR_OVERRIDE", "C:/Program Files (x86)/Steam/vr");
        {
            std::string sPath;
        ...
        HmdError error;
        hmd = VR_Init(&error);
        bool hmdPresent = VR_IsHmdPresent();
    }
...
class RiftApp : public GlfwApp, public SteamVrApp {
public:
...
protected:
    virtual GLFWwindow * createRenderingTarget(uvec2 & outSize, ivec2 & outPosition) {
        return createVrRenderingWindow(hmd, outSize, outPosition);
    }
...
public:
    ExampleApp() {
        modelview = glm::lookAt(glm::vec3(0, 0, ipd * 5.0f), glm::vec3(0), glm::vec3(0, 1, 0));
    }
